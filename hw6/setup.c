...
static void build_tree(BIGNUM **secrets, int n, BN_CTX *ctx,
                       const BIGNUM *p, const BIGNUM *g, BIGNUM **out_key,
                       BIGNUM **out_bk, BIGNUM **leaf_bks, int *leaf_count,
...
int main(int argc, char *argv[])
{
    ...
    int n = argc - 3;
    BIGNUM **secrets = malloc((size_t)n * sizeof(*secrets));

    // Part 2
    for (int i = 0; i < n; i++)
    {
        char *seed = Read_File(argv[i + 3], &tmp_len);
        unsigned char digest[SHA256_DIGEST_LENGTH];

        SHA256((const unsigned char *)seed, strlen(seed), digest);
        secrets[i] = BN_bin2bn(digest, SHA256_DIGEST_LENGTH, NULL);
    ...
    Write_File("group_key_setup.txt", group_key_hex);
    OPENSSL_free(group_key_hex);

    // Part 6
    FILE *bk_file = fopen("blinded_keys_setup.txt", "w");

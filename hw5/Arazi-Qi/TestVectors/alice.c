...
int main(int argc, char **argv)
{
	...
	if (!read_bn_hex(argv[1], &x_a)){
		fprintf(stderr, "Failed argv[1].\n");
		goto cleanup;
	}
	U_a = EC_POINT_new(group);
	U_b = EC_POINT_new(group);
	D = EC_POINT_new(group);
	if (U_a == NULL || U_b == NULL || D == NULL){
		fprintf(stderr, "Failed argv[2].\n");
		goto cleanup;
	}
	...
	if (!read_point_hex(argv[4], group, &U_b)){
		fprintf(stderr, "Failed argv[4].\n");
		goto cleanup;
	}
	if (!read_point_hex(argv[5], group, &D)){
		fprintf(stderr, "Failed argv[5].\n");
		goto cleanup;
	}
	/* =====================================================
	...
	if (!read_point_hex("bob_ephemeral_Eb.txt", group, &E_b)){
		fprintf(stderr, "Alice written. Bob not found\n");
		ret = EXIT_SUCCESS;
		goto cleanup;
	}
	/* =====================================================

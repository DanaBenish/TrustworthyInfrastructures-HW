...
int main(int argc, char **argv)
{
	...
	if(!init_group(&group, &q)) {
		fprintf(stderr, "Error: Failed to initialize group\n");
		exit(EXIT_FAILURE);
	}
	...
	if(!read_bn_hex(argv[1], &msk)) {
		fprintf(stderr, "Error: Failed to read master secret from file %s\n", argv[1]);
		exit(EXIT_FAILURE);
	}
	...
	EC_POINT_mul(group, mpk, NULL, P, msk, ctx);
	...
	write_point_hex("mpk.txt", group, mpk);
